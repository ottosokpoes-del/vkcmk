...
// Update grader
app.put('/api/graders/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const graderData = req.body;
    ...
    await db.execute(
      `UPDATE graders SET 
        title = ?, brand = ?, model = ?, price = ?, year = ?, operating_hours = ?, fuel = ?, transmission = ?,
        location = ?, seller_type = ?, images = ?, description = ?, technical_specs = ?, features = ?, safety = ?,
        is_new = ?, is_sold = ?, is_featured = ?, listing_date = ?, stock_country = ?
      WHERE id = ?`,
      [
        graderData.title,
        graderData.brand || null,
    ...
    res.json({ message: 'Grader updated successfully' });
  } catch (error) {
    console.error('Update grader error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Delete grader
app.delete('/api/graders/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    await db.execute('DELETE FROM graders WHERE id = ?', [id]);
    res.json({ message: 'Grader deleted successfully' });
  } catch (error) {
    console.error('Delete grader error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
app.put('/api/parts/:id', authenticateToken, async (req, res) => {
  try {
    ...
    await db.execute(
      `UPDATE parts SET 
        title = ?, brand = ?, category = ?, price = ?, part_number = ?, compatible_models = ?, images = ?,
        description = ?, specifications = ?, is_new = ?, is_sold = ?, stock_quantity = ?, listing_date = ?, stock_country = ?
      WHERE id = ?`,
      [
        partData.title,
        partData.brand,
...
// Update part stock
app.patch('/api/parts/:id/stock', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await db.execute(
      'UPDATE parts SET stock_quantity = ? WHERE id = ?',
      [stockQuantity, id]
    );
